...
static void usage(int argc, char **argv)
{
	...
	fprintf(stderr,
		"       %s [OPTIONS] [<partition>|<disk> <part number>]\n",
		argv[0]);
...
static int get_driver(map_t *map, int partition, char *appledriver)
{
	...
	if (strncmp(map_get_partition_type(map), "Apple_Driver",
		    strlen("Apple_Driver")) != 0) {
		fprintf(stderr,
	...
	map_get_bootinfo(map, &bootstart, &bootsize, &bootaddr, &bootentry,
			 &checksum, processor);
	...
	if (code == NULL) {
		fprintf(stderr,
			"ERROR: cannot malloc() to load driver in memory\n");
		return -1;
	}
...
int main(int argc, char **argv)
{
	...
	if ((action & ACTION_GET) && (action & ACTION_PUT)) {
		fprintf(stderr,
			"You should use --get-driver OR --put-driver\n");
		return 1;
	}
	...
	if (ret != partition - 1) {
		fprintf(stderr, "ERROR: cannot read partition %d info\n",
			partition);
		return 5;
	}
	...
	if (ret != partition - 1) {
		fprintf(stderr, "ERROR: cannot write partition %d info\n",
			partition);
		return 8;
	}
