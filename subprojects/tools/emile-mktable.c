			 ...
			 unsigned long force_size)
{
	...
	write_long((u_int32_t *)&map512[current].PartBlkCnt,
		   PARTITION_TABLE_LEN);
	...
	write_long((u_int32_t *)&map512[current].BootCksum,
	...
	write_short((u_int16_t *)&map512[current].Sig, MAP_SIGNATURE);
	write_long((u_int32_t *)&map512[current].MapBlkCnt, MAP_BLOCK_COUNT);
	write_long((u_int32_t *)&map512[current].PyPartStart,
		   DRIVER_BASE + driver_block_count);
...
int main(int argc, char **argv)
{
	...
	while (1) {
		c = getopt_long(argc, argv, "he:f:", long_options,
				&option_index);
		if (c == -1)
			break;
		switch (c) {
