...
static int set_output(char *image, unsigned int enable_mask,
		      unsigned int disable_mask, unsigned int bitrate0,
		      int datasize0, int parity0, int stopbits0,
		      unsigned int bitrate1, int datasize1, int parity1,
		      int stopbits1, int gestaltid)
{
	int drive, second, size;
	int fd;
	int ret;
	...
	if (enable_mask & STDOUT_MODEM) {
		sprintf(property, "%d%c%d+%d", bitrate0, parity[parity0],
			datasize0, stopbits0);
		config_set_property(configuration, "modem", property);
	}
	if (enable_mask & STDOUT_PRINTER) {
		sprintf(property, "%d%c%d+%d", bitrate1, parity[parity1],
			datasize1, stopbits1);
...
int main(int argc, char **argv)
{
	...
	ret = set_output(image, enable_mask, disable_mask, bitrate0, datasize0,
			 parity0, stopbits0, bitrate1, datasize1, parity1,
			 stopbits1, gestaltid);
