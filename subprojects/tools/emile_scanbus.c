...
void diskinfo(char *devname)
{
	...
	if ((ret != -1) && verbose) {
		printf(" block size: %d, blocks number: %d (", block_size,
		       block_count);
	...
	for (j = 0; j < map_get_driver_number(map); j++) {
		...
		map_get_driver_info(map, j, &block, &size, &type);
		printf("     %d: base: %d size: %d ", j,
		       block * block_size / 512, size * block_size / 512);
		printf("type: ");
		switch (type) {
		...
		part = map_seek_driver_partition(map, block * block_size / 512);
		if (part == -1) {
		...
		printf(" <%d: %s [%s], ", part + 1, map_get_partition_name(map),
	...
	for (j = 0; j < map_get_number(map); j++) {
		...
		if (map_partition_is_startup(map))
			...
			printf("     ");
		printf("%d: ", j + 1);
		printf("%16s [%-16s] ", map_get_partition_name(map),
		       map_get_partition_type(map));
		...
		if (verbose) {
			...
			int checksum;
			char processor[16];
			if (*(unsigned long *)(map_get_partition_type(map) +
					       28))
				printf("                 patch: %s\n",
			...
			if (ret != -1) {
				...
				map_get_bootinfo(map, &bootstart, &bootsize,
						 &bootaddr, &bootentry,
						 &checksum, processor);
				printf("                 "
