	       ...
	       int second_offset, int second_size)
{
	...
	ret = emile_first_set_param(fd, tune_mask, drive_num, second_offset,
				    second_size);
...
int first_tune_scsi(char *image, int drive_num, int second_offset, int size)
{
	...
	if (drive_num == -1) {
		...
		unsigned short *max_blocks =
			(unsigned short *)(((char *)&firstBlock) + 1022);
		unsigned long *second_size =
			(unsigned long *)(((char *)&firstBlock) + 1018);
		unsigned short *unit_id =
			(unsigned short *)(((char *)&firstBlock) + 1016);
		unsigned short *block_size =
			(unsigned short *)(((char *)&firstBlock) + 1014);
		...
		ret = emile_first_set_param_scsi_extents(
			fd, drive_num, second_offset, size, BLOCK_SIZE);
...
int main(int argc, char **argv)
{
	...
	if (path && tune_mask) {
		fprintf(stderr,
			"ERROR: you cannot supply second path and size, offset or drive number\n");
		usage(argc, argv);
		return 1;
	}
	...
	if (path)
		...
		ret = first_tune_scsi(image, drive_num, second_offset,
				      second_size);
	else {
		second_offset = (second_offset + 0x1FF) & 0xFFFFFE00;
		second_size = (second_size + 0x1FF) & 0xFFFFFE00;
		ret = first_tune(image, tune_mask, drive_num, second_offset,
				 second_size);
	}
	switch (ret) {
	case 0:
