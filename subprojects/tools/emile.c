...
static struct option long_options[] = { { "scanbus", 0, NULL, ARG_SCANBUS },
...
static int open_map_of(char *dev_name, int flags, map_t **map, int *partition)
{
	int ret;
	int disk;
	char disk_name[16];
...
static int restore_bootblock(char *dev_name, char *filename)
{
	...
	if (!check_is_EMILE_bootblock(dev_name)) {
		fprintf(stderr,
			"ERROR: cannot restore bootblock over non-EMILE bootblock\n");
		return -1;
	}
...
static int add_file(int8_t *configuration, char *index, char *property,
		    char *path, char *map_path)
{
	...
	if (emile_is_url(path)) {
		...
		config_set_indexed_property(configuration, "title", index,
					    property, path);
		return 0;
	}
	...
	if (verbose)
		printf("    %s %s (%s = %s)\n", property, path, map_path,
		       map_info);

	config_set_indexed_property(configuration, "title", index, property,
				    map_info);

	if ((action & ACTION_TEST) != 0)
		fprintf(stderr,
			"WARNING: test mode, you must remove manually %s\n",
			map_path);
...
static int8_t *set_config_no_fs(char *config_path)
{
	...
	char property2[1024];
	char property3[1024];
	static char *prolog[] = { "gestaltID", "default", "timeout",
				  "vga",       "modem",	  "printer" };
	...
	for (i = 0; i < sizeof(prolog) / sizeof(char *); i++) {
		if (config_get_property(conffile, prolog[i], property) != -1)
			config_set_property(configuration, prolog[i], property);
	}

	current = 0;
	while (1) {
		ret = config_get_property(conffile + current, "title",
					  property);
		if (ret == -1)
			break;
		...
		current += ret;
		current =
			config_get_next_property(conffile, current, NULL, NULL);

		ret = config_get_indexed_property(conffile, "title", property,
		...
		if (ret != -1) {
			if (emile_is_url(property2)) {
				...
				ret = emile_scsi_create_container(fd, &unit_id,
								  container, 2);
				close(fd);
				if (ret == -1) {
					fprintf(stderr,
				...
				sprintf(chainloader, "block:(sd%d)0x%x,0x%jx",
					unit_id, container->blocks[0].offset,
					st.st_size);
				free(container);
				config_set_indexed_property(configuration,
							    "title", property,
		...
		ret = config_get_indexed_property(conffile, "title", property,
						  "kernel", property2);
		if (ret != -1) {
			ret = config_get_indexed_property(conffile, "title",
							  property,
							  "kernel_map",
							  property3);

			ret = add_file(configuration, property, "kernel",
				       property2, ret == -1 ? NULL : property3);
			if (ret == -1) {
				fprintf(stderr, "ERROR: cannot add kernel %s\n",
					property2);
				free(conffile);
				free(configuration);
				return NULL;
		...
		if (ret != -1) {
			...
			ret = add_file(configuration, property, "initrd",
				       property2, ret == -1 ? NULL : property3);
			if (ret == -1) {
				free(configuration);
				fprintf(stderr, "ERROR: cannot add initrd %s\n",
					property3);
				fprintf(stderr,
		...
		ret = config_get_indexed_property(conffile, "title", property,
						  "args", property2);
		if (ret != -1) {
			config_set_indexed_property(configuration, "title",
						    property, "args",
						    property2);
			if (verbose)
				printf("    args %s\n", property2);
	...
	if (strlen((char *)configuration) > 1023) {
		...
		ret = add_file(configuration, NULL, "configuration", bootconfig,
			       NULL);
		if (ret == -1) {
			free(configuration);
			fprintf(stderr,
...
int main(int argc, char **argv)
{
	...
	while (1) {
		c = getopt_long(argc, argv, "vhtc:", long_options,
				&option_index);
		if (c == -1)
			break;
		switch (c) {
	...
	if (fstat(fd, &st) == -1) {
		fprintf(stderr, "ERROR: cannot fstat file %s\n", config_path);
		return 2;
	}
	...
	if (ret == -1) {
		fprintf(stderr,
			"ERROR: you must specify in %s a partition to set\n"
			"       EMILE bootblock\n",
			config_path);
		fprintf(stderr,
	...
	if (ret != -2) {
		if (ret == -1) {
			fprintf(stderr,
				"ERROR: cannot check if Apple_Driver exists\n");
	...
	if (ret == -1) {
		fprintf(stderr, "ERROR: missing first_level in config file\n");
		return 2;
	}
	...
	if (ret == -1) {
		fprintf(stderr, "ERROR: missing second_level in config file\n");
		return 2;
	}
