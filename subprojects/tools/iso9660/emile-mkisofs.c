...
#define COMMAND \
	"/usr/bin/mkisofs %s -hfs -joliet -R -boot-hfs-file %s -graft-points -o %s"
...
static int create_apple_driver(char *temp, char *appledriver, char *first_level)
{
	...
	fstat(fd_driver, &st);
	driver_size =
		((st.st_size + DRIVER_SIZE - 1) / DRIVER_SIZE) * DRIVER_SIZE;
	...
	write_short((u_int16_t *)&block0.DrvInfo[0].Size,
		    (driver_size + BLOCKSIZE - 1) / BLOCKSIZE);
	...
	write_long((u_int32_t *)&map512.PartBlkCnt,
		   (driver_size + 512 - 1) / 512);
	...
	write_long((u_int32_t *)&map512.PartStatus,
		   kPartitionAUXIsValid | kPartitionAUXIsAllocated |
			   kPartitionAUXIsInUse | kPartitionAUXIsBootValid |
			   kPartitionAUXIsReadable | kPartitionAUXIsWriteable |
	...
	write_long((u_int32_t *)&map512.BootCksum,
		   map_checksum(driver, st.st_size));
	strncpy(map512.Processor, "68000", 16);
	write_long((u_int32_t *)map512.Pad, kSCSICDDriverSignature);

	map2048 = map512;
	write_long((u_int32_t *)&map2048.PartBlkCnt,
		   (driver_size + BLOCKSIZE - 1) / BLOCKSIZE);
...
static int get_second_position(char *image, char *name, int *second_offset,
			       int *second_size)
{
	stream_t stream;
	...
	ret = iso9660_create_stream(&stream, image);
	if (ret) {
		fprintf(stderr, "ERROR: Cannot open \"%s\"\n", image);
		return 1;
	}

	ret = stream_file_open(&stream, name);
	if (ret) {
...
static int set_second(char *image, int second_offset, char *kernel_image,
		      char *cmdline, char *ramdisk)
{
	int fd;
	int ret;
...
static int set_first(char *image, int drive_num, int second_offset,
		     int second_size)
{
	int fd;
	map_t *map;
	...
	device_create_stream(&stream, image);
	...
	ret = emile_first_set_param_scsi_extents(fd, 3, second_offset / 4,
						 second_size, BLOCKSIZE);
	close(fd);
...
int main(int argc, char **argv)
{
	...
	while (1) {
		c = getopt_long(argc, argv,
				"hvf:s:k:r:d:i:a:e:c:", long_options,
				&option_index);
		if (c == -1)
			break;
		switch (c) {
	...
	if (image == NULL) {
		fprintf(stderr,
			"ERROR: you must provide a filename to write image\n");
		usage(argc, argv);
		return 1;
	}
	...
	if (second_level) {
		...
		if (verbose)
			sprintf(buffer, COMMAND " %s=%s", "", temp, image,
				second_on_iso, second_level);
		else
			sprintf(buffer, COMMAND " %s=%s", "-quiet", temp, image,
				second_on_iso, second_level);
	} else {
		if (verbose)
	...
	if (second_level) {
		if (get_second_position(image, second_on_iso, &second_offset,
					&second_size)) {
			return 2;
		}
		...
		if (conffile)
			...
			set_second(image, second_offset, kernel_image, cmdline,
				   ramdisk);
	}
