...
static int get_info(char *image, int verbose)
{
	...
	char title[1024];
	int index;
	char *known_properties[] = { "root", "kernel",	    "initrd",
				     "args", "chainloader", NULL };
	int i;
	int current;
	int res;
	int ret;
	...
	for (index = 0; index < 20; index++) {
		res = config_get_property(configuration + current, "title",
					  title);
		if (res == -1) {
			if (index)
				break;
		...
		for (i = 0; known_properties[i] != NULL; i++) {
			if (config_get_indexed_property(
				    configuration, (res == -1) ? NULL : "title",
				    title, known_properties[i], property) != -1)
				printf("    %s %s\n", known_properties[i],
				       property);
		}
	}
		      ...
		      char *first_level, char *second_level)
{
	...
	static char *prolog[] = { "gestaltID", "default", "timeout",
				  "vga",       "modem",	  "printer" };
	static char *known_properties[] = { "root", "kernel", "initrd", "args",
					    "chainloader" };
	...
	if (first_level == NULL) {
		if (config_get_property(conffile, "first_level", property) ==
		    -1)
	...
	if (second_level == NULL) {
		if (config_get_property(conffile, "second_level", property2) ==
		    -1)
	...
	current = 0;
	while (1) {
		res = config_get_property(conffile + current, "title",
					  property);
		if (res == -1)
			break;
		...
		has_root = 0;
		for (i = 0; i < sizeof(known_properties) / sizeof(char *);
		     i++) {
			res = config_get_indexed_property(conffile, "title",
							  property,
							  known_properties[i],
							  property2);
			if (res == -1)
			...
			if (strcmp(known_properties[i], "root") == 0) {
				...
				char *url;
				if (strcmp(known_properties[i], "kernel") ==
				    0) {
					if (kernel_ondisk == NULL)
						kernel_ondisk =
							emile_floppy_add(
								fd, property2);
					url = kernel_ondisk;
				} else if (strcmp(known_properties[i],
						  "initrd") == 0) {
	...
	if (strlen((char *)configuration) > 1023) {
		...
		fprintf(stderr,
			"WARNING: configuration doesn't fit in second level\n"
...
int main(int argc, char **argv)
{
	...
	if (config_path != NULL) {
		...
		return set_config(image, verbose, config_path, first_level,
				  second_level);
	}
	...
	ret = emile_floppy_create_image(first_level, second_level, kernel_image,
					ramdisk, image);
	switch (ret) {
	case 0:
		break;
