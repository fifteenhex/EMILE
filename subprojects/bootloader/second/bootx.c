...
void bootx_init(char *command_line, char *ramdisk_start,
		unsigned long ramdisk_size)
{
	...
	bootx_infos.kernelParamsOffset =
		(long)command_line - (long)&bootx_infos;

	for (i = 0; i < memory_map.bank_number; i++) {
		bootx_infos.physMemoryMap[i].physAddr =
