...
void serial_enable(int port, int bitrate, int parity, int datasize,
		   int stopbits)
{
	int res;

	res = OpenDriver(c2pstring(".AOut"), &out_refnum[port]);
	if (res != noErr) {
		printf("Cannot open output port (%d)\n", res);
	} else {
		res = setserial(out_refnum[port], bitrate, datasize, parity,
				stopbits);
		if (res != noErr) {
...
#ifdef USE_CLI
	res = OpenDriver(c2pstring(".AIn"), &in_refnum[port]);
	if (res != noErr) {
		printf("Cannot open modem input port (%d)\n", res);
	} else {
		res = setserial(in_refnum[port], bitrate, datasize, parity,
				stopbits);
		if (res != noErr) {
