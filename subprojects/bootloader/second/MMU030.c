...
#define GET_RP_LIMIT(CRP, max, min)            \
	if (CRP[0] & 0x80000000) {             \
		min = (CRP[0] >> 16) & 0x7FFF; \
		max = RP_LIMIT_MAX;            \
	} else {                               \
...
#define GET_TD_LF_LIMIT(PD0, PD1, max, min) \
	if (PD0 & 0x80000000) {             \
		min = (PD0 >> 16) & 0x7FFF; \
		max = RP_LIMIT_MAX;         \
	} else {                            \
...
static int decode_8_PD(unsigned long *pageBase, unsigned long *pageMask,
		       unsigned long *attr, unsigned long logicalAddr,
		       unsigned long TI, unsigned long PD0, unsigned long PD1);

static int decode_4_PD(unsigned long *pageBase, unsigned long *pageMask,
		       unsigned long *attr, unsigned long logicalAddr,
		       unsigned long TI, unsigned long PD)
{
	int dt;
	int TIA;
	unsigned long root;
	...
	case DT_VALID_4_BYTE:
		...
		*pageMask = (*pageMask) >> TIA;
		...
		return decode_4_PD(pageBase, pageMask, attr, logicalAddr << TIA,
				   TI << 4, MMU030_read_phys(root + index * 4));
	...
	case DT_VALID_8_BYTE:
		...
		*pageMask = (*pageMask) >> TIA;
		...
		return decode_8_PD(pageBase, pageMask, attr, logicalAddr << TIA,
				   TI << 4, MMU030_read_phys(root + index * 8),
...
int MMU030_logical2physicalAttr(unsigned long logicalAddr,
	...
	*attr = 0;
	...
	case DT_VALID_4_BYTE:
		...
		ret = decode_4_PD(&pageBase, &pageMask, attr,
				  logicalAddr << (is + TIA), GET_TC_TI(TC) << 4,
...
int MMU030_logical2physical(unsigned long logicalAddr,
			    unsigned long *physicalAddr)
{
	unsigned long attr;
...
static void dump_8_PD(int shift, unsigned long PD0, unsigned long PD1)
{
	...
	if (max - min < 0)
		printf("ERROR ! max(%d) - min(%d) < 0, TIA = %d, ", max, min,
		       TIA);
...
void MMU030_dump_table()
{
	...
	case DT_VALID_4_BYTE:
		for (i = 0; i < max - min; i++) {
			...
			printf("0x%08lx -> ", (unsigned long)(i + min)
						      << (32 - TIA));
			dump_4_PD(0, PD);
		}
		break;
