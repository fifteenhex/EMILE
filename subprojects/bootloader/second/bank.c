...
static void bank_add_mem(unsigned long logiAddr, unsigned long physAddr,
			 unsigned long size)
{
	int i;
	int j;
	...
	for (i = 0; i < memory_map.bank_number; i++) {
		if ((memory_map.bank[i].physAddr + memory_map.bank[i].size ==
		     physAddr) &&
...
#ifdef ARCH_M68K
void m68k_init_memory_map(void)
{
#ifdef USE_MMU
