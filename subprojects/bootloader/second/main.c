...
retry:
	...
	kernel = load_kernel(econfig.kernel, bootstrap_size, &start_mem,
			     &entry_point, &kernel_size);
	if (kernel == NULL) {
	...
	if (arch_type == gestalt68k) {
		...
		memcpy((char *)kernel + kernel_size + bootstrap_size -
			       enter_size,
		       (char *)enter_kernel, enter_size);

		end_enter_kernel =
			(unsigned long)kernel + kernel_size + bootstrap_size;
		enter_kernel = (unsigned long)kernel + bootstrap_size -
			       enter_size + kernel_size;
		...
		if (mmu_type == gestaltNoMMU) {
			...
			bootinfo_init(econfig.command_line, ramdisk_start,
				      ramdisk_size);
...
#else
		{
			ret = logical2physical((unsigned long)kernel,
					       &physImage);
			...
			aligned_size = boot_info.memory[0].addr &
				       (KERNEL_ALIGN - 1);
			if (aligned_size > 0) {
				aligned_size = KERNEL_ALIGN - aligned_size;
				aligned_addr =
					boot_info.memory[0].addr + aligned_size;
				aligned_size =
			...
			if ((ret == 0) &&
			    (enter_kernel != (unsigned long)entry)) {
				unsigned long logi;
				unsigned long size =
				...
				memcpy((char *)logi, (char *)enter_kernel,
				       size);
				memcpy((char *)entry, (char *)enter_kernel,
				       size);
			}
		}
#endif /* USE_MMU */
...
#define BOOT_KERNEL_STACK_SIZE 65536
		regs.GPR[1] =
			(u_int32_t)malloc_contiguous(BOOT_KERNEL_STACK_SIZE) +
		...
		regs.GPR[8] = ((((&_bootstrap_end - &_bootstrap_start) + 4095)
				<< 12) >>
			       12);
