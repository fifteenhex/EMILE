...
static int MacHasHardware(unsigned long gestaltBit)
{
	long r;
...
void bootinfo_init(char *command_line, char *ramdisk_start,
...
void set_kernel_bootinfo(char *dst)
{
	...
	if (boot_info.bi_mac.videoaddr) {
		...
		dst = add_v2_boot_record(dst, V2_BI_MAC_VDEPTH, sizeof(l1),
					 &l1);
		...
		dst = add_v2_boot_record(dst, V2_BI_MAC_VLOGICAL, sizeof(l1),
					 &l1);
	}
	...
	if (boot_info.bi_mac.HwMap & HW_MAP_VIA1) {
		...
		dst = add_v2_boot_record(dst, V2_BI_MAC_VIA1BASE, sizeof(l1),
					 &l1);
	}
	...
	if (boot_info.bi_mac.HwMap &
		...
		dst = add_v2_boot_record(dst, V2_BI_MAC_VIA2BASE, sizeof(l1),
					 &l1);
		dst = add_v2_boot_record(dst, V2_BI_MAC_VIA2TYPE, sizeof(l2),
					 &l2);
	}
	...
	if (boot_info.bi_mac.HwMap & HW_MAP_ASC) {
		...
		dst = add_v2_boot_record(dst, V2_BI_MAC_ASCBASE, sizeof(l1),
					 &l1);
	}
	...
	if (boot_info.bi_mac.HwMap & HW_MAP_SCSI5380) {
		...
		dst = add_v2_boot_record(dst, V2_BI_MAC_SCSI5380, sizeof(l1),
					 &l1);
	}
	...
	if (boot_info.bi_mac.HwMap & HW_MAP_SCSI5380DMA) {
		...
		dst = add_v2_boot_record(dst, V2_BI_MAC_SCSIDMA, sizeof(l1),
					 &l1);
	}
	...
	if (boot_info.bi_mac.HwMap & HW_MAP_SCSI5396) {
		...
		dst = add_v2_boot_record(dst, V2_BI_MAC_SCSI5396, sizeof(l1),
					 &l1);
	}
	...
	if (boot_info.bi_mac.HwMap & HW_MAP_IDE_PB) {
		l1 = V2_IDE_PB;
		dst = add_v2_boot_record(dst, V2_BI_MAC_IDETYPE, sizeof(l1),
					 &l1);
		l2 = V2_IDE_BASE;
		dst = add_v2_boot_record(dst, V2_BI_MAC_IDEBASE, sizeof(l2),
					 &l2);
	...
	if (boot_info.bi_mac.HwMap & HW_MAP_NUBUS) {
		...
		dst = add_v2_boot_record(dst, V2_BI_MAC_SLOTMASK, sizeof(l1),
					 &l1);
	}
