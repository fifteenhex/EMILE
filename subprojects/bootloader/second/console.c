...
int console_select(int timeout)
{
...
int console_status_request(void)
{
	int car;
	int status;
	int i;
...
#if 1
static int console_picolibc_putc(char c, FILE *file)
{
	(void)file;
...
static int console_picolibc_getc(FILE *file)
{
	unsigned char c = '\0';
	(void)file;
...
static FILE __stdio = FDEV_SETUP_STREAM(
	console_picolibc_putc, console_picolibc_getc, NULL, _FDEV_SETUP_RW);
