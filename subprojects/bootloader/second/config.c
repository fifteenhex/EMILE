...
#define MSG_STATE_0                                                  \
	console_set_cursor_position(win.l + win.h + 1, 1);           \
...
static int8_t *open_config(emile_l2_header_t *info)
{
	...
	if (config_get_property(configuration, "configuration", property) !=
	    -1) {
...
int read_config(emile_l2_header_t *info, emile_config_t *econfig)
{
	...
	if (config_get_property(configuration, "modem", property) != -1) {
		decode_serial(property, &bitrate, &parity, &datasize,
			      &stopbits);
		serial_enable(SERIAL_MODEM_PORT, bitrate, parity, datasize,
			      stopbits);
	}
	...
	if (config_get_property(configuration, "printer", property) != -1) {
		decode_serial(property, &bitrate, &parity, &datasize,
			      &stopbits);
		serial_enable(SERIAL_PRINTER_PORT, bitrate, parity, datasize,
			      stopbits);
	}
	...
	for (index = 0; index < MAX_KERNELS; index++) {
		...
		for (i = 0; known_properties[i] != NULL; i++) {
			if (config_get_indexed_property(
				    ...
				    title[index], known_properties[i],
				    property) == -1)
				continue;
	...
	console_clear();
	sprintf(line,
		"EMILE v" VERSION " (c) 2004-2012 Laurent Vivier (%ld kB)",
		bank_mem_avail() / 1024);
	len = strlen(line);
	if (len < c)
		len = (c - len) / 2 + 1;
	...
	while (state != -1) {
		...
		case 1: /* select parameter */
			...
			case 'n':
			case 'N':
				properties[choice][prop_nb[choice]] =
					strdup("");
		...
		case 2: /* edit parameter */
			...
			strncpy(property, properties[choice][list.current],
				COMMAND_LINE_LENGTH);
			free(properties[choice][list.current]);
