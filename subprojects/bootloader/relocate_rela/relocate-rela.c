...
static const bool debug_en = true;
...
static bool supported_rela(Elf64_Rela *rela)
{
	...
	default:
		fprintf(stderr,
			"warning: unsupported relocation type %" PRIu32
			" at %" PRIx64 "\n",
			type, rela->r_offset);
...
static int decode_elf64(FILE *felf, char **argv)
{
	...
	section_header_size =
		le16_to_cpu(header.e_shentsize) * le16_to_cpu(header.e_shnum);

	sh_table = malloc(section_header_size);
	if (!sh_table) {
		fprintf(stderr,
			"%s: Cannot allocate space for section header\n",
			argv[0]);
		fclose(felf);
		return 26;
	}
	...
	if (ret) {
		fprintf(stderr,
			"%s: Can't set pointer to section header: %x/%lx\n",
...
static int decode_elf32(FILE *felf, char **argv)
{
	...
	section_header_size =
		elf16_to_cpu(header.e_shentsize) * elf16_to_cpu(header.e_shnum);
...
static int rela_elf64(char **argv, FILE *f)
{
	...
	if ((rela_end - rela_start) % sizeof(Elf64_Rela)) {
		fprintf(stderr,
			"%s: rela size isn't a multiple of Elf64_Rela\n",
			argv[0]);
		return 3;
	}
...
static bool supported_rela32(Elf32_Rela *rela, uint32_t *type)
{
	...
	*type = rela->r_info & mask;
	...
	fprintf(stderr,
		"warning: unsupported relocation type %" PRIu32 " at %" PRIx32
		"\n",
		*type, rela->r_offset);
...
static int rela_elf32(char **argv, FILE *f)
{
	...
	if ((rela_end - rela_start) % sizeof(Elf32_Rela)) {
		fprintf(stderr,
			"%s: rela size isn't a multiple of Elf32_Rela\n",
			argv[0]);
		return 3;
	}
	...
	for (i = 0; i < num; i++) {
		...
		if (fseek(f, pos, SEEK_SET) < 0) {
			fprintf(stderr,
				"%s: %s: seek to %" PRIx32 " failed: %s\n",
				argv[0], argv[1], pos, strerror(errno));
		}

		if (fread(&rela, sizeof(rela), 1, f) != 1) {
			fprintf(stderr,
				"%s: %s: read rela failed at %" PRIx32 "\n",
				argv[0], argv[1], pos);
			return 4;
		}

		debug("Rela:\toffset:\t%" PRIx32 " r_info:\t%" PRIu32
		      " r_addend:\t%" PRIx32 "\n",
		...
		debug("SWRela:\toffset:\t%" PRIx32 " r_info:\t%" PRIu32
		      " r_addend:\t%" PRIx32 "\n",
		      swrela.r_offset, swrela.r_info, swrela.r_addend);
		...
		if ((machine == EM_M68K && type == R_68K_RELATIVE) ||
			...
			if (fwrite(&rela.r_addend, sizeof(rela.r_addend), 1,
				   f) != 1) {
				fprintf(stderr,
					"%s: %s: write failed at %" PRIx32 "\n",
					argv[0], argv[1], addr);
				return 4;
			}
			...
			if (fread(&symbols, sizeof(symbols), 1, f) != 1) {
				fprintf(stderr,
					"%s: %s: read symbols failed at %" PRIx32
					"\n",
					argv[0], argv[1], pos_dyn);
				return 4;
			}
