...
static inline OSErr FlushInstructionCache(void)
{
	...
	asm volatile(HWPriv(_FlushInstructionCache)::: UNPRESERVED_REGS);
...
static inline OSErr FlushDataCache(void)
{
	...
	asm volatile(HWPriv(_FlushDataCache)::: UNPRESERVED_REGS);
...
static inline OSErr DisableExtCache(void)
{
	...
	asm volatile(HWPriv(_DisableExtCache)::: UNPRESERVED_REGS);
...
static inline OSErr FlushExtCache(void)
{
	...
	asm volatile(HWPriv(_FlushExtCache)::: UNPRESERVED_REGS);
...
static inline OSErr FlushCodeCacheRange(void *address, unsigned long count)
{
	...
	asm volatile(
		"move.l %0, %%a0\n"
		"move.l %1, %%a1\n" HWPriv(_FlushCodeCacheRange)::"g"(address),
		"g"(count)
		: UNPRESERVED_REGS);
