...
int config_set_indexed_property(int8_t *configuration, char *index_name,
				...
				char *property)
{
	...
	if (last_index == -1) {
		...
		last_index = config_find_indexed_property(
			configuration, index_name, index_property, NULL, NULL);
		if (last_index == -1) {
			...
			last_index = config_get_next_property(
				configuration, last_index, NULL, NULL);
			if (last_index != -1)
				last_index = config_find_entry(
					configuration + last_index, index_name,
					NULL);
			if (last_index == -1) {
				last_index = strlen((char *)configuration);
				if (last_index > 0)
	...
	memmove(configuration + last_index + len_new, configuration + index,
		len);

	if (last_index > 0)
		configuration[last_index - 1] = '\n';
	sprintf((char *)configuration + last_index, "%s %s", name, property);
