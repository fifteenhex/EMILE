...
static ssize_t copy_file(int fd, char *file)
{
	...
	for (;;) {
		...
		if (size_read == FLOPPY_SECTOR_SIZE) {
			...
			memset(buffer + size_read, 0,
			       FLOPPY_SECTOR_SIZE - size_read);
			size_written = write(fd, buffer, FLOPPY_SECTOR_SIZE);
			total += size_written;
			if (size_written != FLOPPY_SECTOR_SIZE) {
				close(source);
...
{
	return path &&
	       ((strncmp(path, "iso9660:", strlen("iso9660:")) == 0) ||
...
int emile_floppy_create(char *image, char *first_level, char *second_level)
{
	...
	ret = emile_first_set_param(
		fd,
		EMILE_FIRST_TUNE_DRIVE | EMILE_FIRST_TUNE_OFFSET |
			EMILE_FIRST_TUNE_SIZE,
		1, FIRST_LEVEL_SIZE, emile_file_get_size(second_level));
	if (ret != 0) {
		close(fd);
		return ret;
	}
...
int emile_floppy_create_image(char *first_level, char *second_level,
			      char *kernel_image, char *ramdisk, char *image)
{
	int ret;
	int fd;
