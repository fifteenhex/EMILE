...
static int get_device_info(int fd, unsigned short *id,
			   unsigned long *first_block, int *block_size)
{
	int ret;
	char dev_name[16];
	...
	*id = pun;
	...
	if (ret == -1) {
		fprintf(stderr, "%s: ioctl(HDIO_GETGEO) fails: %s", dev_name,
			strerror(errno));
		return -1;
	}
				...
				int maxblocks)
{
	...
	current = 0;
	for (logical = 1; logical < (st.st_size + block_size - 1) / block_size;
	     logical++) {
		physical = logical;
