...
extern int emile_first_set_param_scsi_extents(int fd, int drive_num,
					      int second_offset,
					      int second_size, int blocksize);
struct emile_container *emile_second_create_mapfile(unsigned short *unit_id,
						    char *mapfile,
						    char *kernel);
extern int emile_is_url(char *path);
extern int emile_floppy_create(char *image, char *first_level,
			       char *second_level);
...
extern int emile_second_get_next_property(int8_t *configuration, int index,
					  char *name, char *property);
extern int emile_second_get_property(int8_t *configuration, char *name,
				     char *property);
extern void emile_second_set_property(int8_t *configuration, char *name,
				      char *property);
extern void emile_second_remove_property(int8_t *configuration, char *name);
extern int emile_second_set_param(int fd, char *kernel, char *parameters,
				  char *initrd);
extern int emile_second_get_param(int fd, char *kernel, char *parameters,
				  char *initrd);
extern int emile_scsi_get_dev(int fd, int *driver, int *disk, int *partition);
extern int emile_get_dev_name(char *buffer, int driver, int disk,
			      int partition);
extern int emile_scsi_get_rdev(char *dev_name, int *driver, int *disk,
			       int *partition);
#endif
