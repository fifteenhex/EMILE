...
size_t block_read(void *_file, void *ptr, size_t size)
{
	...
	int read = 0;
	int ret;
	int blocksize = file->device->ops->get_blocksize(file->device->data);

	while (size != 0) {
		block_nb = (file->base + file->offset / blocksize) /
			   file->buffer_size;
		block_offset = (file->base - block_nb * file->buffer_size) *
				       blocksize +
			       file->offset;
		if (block_nb != file->current) {
			ret = file->device->ops->read_sector(
				file->device->data,
				block_nb * file->buffer_size, file->buffer,
