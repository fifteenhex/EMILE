...
void iso9660_name(stream_VOLUME *volume, struct iso_directory_record *idr,
		  char *buffer)
{
	int j;
...
stream_VOLUME *iso9660_mount(device_io_t *device)
{
	...
	device->ops->read_sector(device->data, 16, &ipd, sizeof(ipd));
	...
	jpd = (struct iso_primary_descriptor *)malloc(
	...
	while ((u_int8_t)jpd->type[0] != ISO_VD_END) {
		...
		if (jpd->unused3[0] == '%' && jpd->unused3[1] == '/' &&
		    (jpd->unused3[3] == '\0' || jpd->unused3[3] == ' ') &&
		    (jpd->unused3[2] == '@' || jpd->unused3[2] == 'C' ||
		     jpd->unused3[2] == 'E')) {
			if (jpd->version[0] == 1)
				goto nextblock;
		}
...
nextblock:
		block++;
		device->ops->read_sector(device->data, block, jpd,
					 sizeof(*jpd));
	}
...
{
	return (struct iso_directory_record *)
		volume->descriptor->root_directory_record;
}
