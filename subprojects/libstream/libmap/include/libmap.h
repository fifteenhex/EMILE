...
#define ASSERT_DD(a)                                  \
	if (sizeof(struct DriverDescriptor) != 512) { \
		a                                     \
	}
...
enum {
	kPatchDriverSignature =
		0x70744452, /* 'ptDR', SCSI and ATA[PI] patch driver */
	...
	kDriveSetupHFSSignature =
		0x44535531, /* 'DSU1', Drive Setup HFS partition */
};
...
#define ASSERT_P(a)                            \
	if (sizeof(struct Partition) != 512) { \
		a                              \
	}
...
extern void *map_open(device_io_t *device);
extern void map_close(void *map);
...
extern int map_geometry(map_t *map, int *block_size, int *block_count);
extern int map_get_driver_number(map_t *map);
extern int map_get_driver_info(map_t *map, int number, int *block, int *size,
			       int *type);
...
extern int map_get_bootinfo(map_t *map, int *bootstart, int *bootsize,
			    int *bootaddr, int *bootentry, int *checksum,
			    char *processor);
extern int map_set_bootinfo(map_t *map, int bootstart, int bootsize,
			    int bootaddr, int bootentry, int checksum,
			    char *processor);
extern int map_set_driver_info(map_t *map, int number, int block, int size,
			       int type);
...
extern int map_read_sector(void *map, off_t block, void *buffer, size_t nb);
extern int map_write_sector(map_t *map, off_t block, char *buffer, size_t nb);
extern int map_get_blocksize(void *map);
extern unsigned short map_checksum(unsigned char *addr, unsigned int length);
extern unsigned short map_checksum_ATA(unsigned char *addr,
				       unsigned int length);
