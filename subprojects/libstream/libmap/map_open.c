...
void *map_open(device_io_t *device)
{
	map_t *map;
	int ret;
	int blocksize = device->ops->get_blocksize(device->data);
	...
	ret = map->device->ops->read_sector(map->device->data, 0, &map->drivers,
					    blocksize);
	if (ret == -1) {
	...
	ret = map->device->ops->read_sector(map->device->data, 1,
