...
static unsigned long seek_block(void *_file)
{
	...
	int block_size = file->device->ops->get_blocksize(file->device->data);

	for (i = 0, current = 0; container->blocks[i].offset != 0; i++) {
		int extent_size = block_size * container->blocks[i].count;
...
size_t container_read(void *_file, void *ptr, size_t size)
{
	container_FILE *file = (container_FILE *)_file;
	int err;
	ssize_t read = 0;
	...
	while (size != 0) {
		...
		if (block_nb != file->current_block) {
			err = file->device->ops->read_sector(file->device->data,
							     block_nb,
							     file->buffer,
							     block_size);
			if (err == -1)
