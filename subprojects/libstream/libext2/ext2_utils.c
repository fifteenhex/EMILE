...
{
	device->ops->read_sector(device->data, 2, super, sizeof(*super));
...
void ext2_read_block(ext2_VOLUME *volume, unsigned int fsblock)
{
	...
	blocksize = volume->device->ops->get_blocksize(volume->device->data);
	block = fsblock * (EXT2_BLOCK_SIZE(volume->super) / blocksize);
	volume->device->ops->read_sector(volume->device->data, block,
					 volume->buffer,
					 EXT2_BLOCK_SIZE(volume->super));
}

void ext2_get_group_desc(ext2_VOLUME *volume, int group_id,
...
int ext2_get_inode(ext2_VOLUME *volume, unsigned int ino,
				 ...
				 unsigned int logical)
{
	...
	if (logical < addr_per_block) {
		...
		physical = __le32_to_cpu(
			((unsigned int *)volume->buffer)[logical]);
		return physical;
	}
	...
	if (logical < addr_per_block * addr_per_block) {
		...
		physical = __le32_to_cpu(
			((unsigned int *)
				 volume->buffer)[logical / addr_per_block]);
	...
	physical = __le32_to_cpu((
		(unsigned int *)volume
			->buffer)[logical / (addr_per_block * addr_per_block)]);
	ext2_read_block(volume, physical);
	logical = logical % (addr_per_block * addr_per_block);
	physical = __le32_to_cpu(
		((unsigned int *)volume->buffer)[logical / addr_per_block]);
	ext2_read_block(volume, physical);
	physical = __le32_to_cpu(
		((unsigned int *)volume->buffer)[logical % addr_per_block]);
	return physical;
...
int ext2_read_data(ext2_VOLUME *volume, struct ext2_inode *inode, off_t offset,
...
off_t ext2_dir_entry(ext2_VOLUME *volume, struct ext2_inode *inode, off_t index,
		     struct ext2_dir_entry_2 *entry)
{
	int ret;

	ret = ext2_read_data(volume, inode, index, (char *)entry,
			     sizeof(*entry));
	if (ret == -1)
		return -1;
...
unsigned int ext2_seek_name(ext2_VOLUME *volume, const char *name)
{
	...
	while (1) {
		...
		while (1) {
			...
			if (ret == 0 && (name[entry.name_len] == 0 ||
					 name[entry.name_len] == '/')) {
				ino = entry.inode;
				break;
			}
