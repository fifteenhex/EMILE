...
static char *get_device(char *path, device_t *device, int *unit, int *partition)
{
	if (*path == '(') {
		int nb;
...
stream_t *stream_open(char *dev)
{
	...
	stream->fs_id = fs, stream->device_id = device;
	stream->unit = unit;
	...
	if (partition != -1) {
		...
		if (map_init(&stream->device, partition) == -1) {
			...
			stream->device.ops->close(stream->volume);
			free(stream);
			return NULL;
		}
#else
