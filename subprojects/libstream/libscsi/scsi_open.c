...
#define ISO_BLOCKS(X) \
	(((X) / device->sector_size) + (((X) % device->sector_size) ? 1 : 0))

static int scsi_get_blocksize(void *_device)
{
	scsi_device_t *device = (scsi_device_t *)_device;
...
static int scsi_read_sector(void *_device, off_t offset, void *buffer,
			    size_t size)
{
	scsi_device_t *device = (scsi_device_t *)_device;
	OSErr err;
...
const struct device_io_ops scsi_ops = {
	.read_sector = scsi_read_sector,
	.close = scsi_close,
	.get_blocksize = scsi_get_blocksize,
};

void *scsi_open(int target)
{
	scsi_device_t *dev;
	...
	if (dev == NULL)
		return NULL;
	dev->target = target;
	dev->capacity = (buff[0] << 24) | (buff[1] << 16) | (buff[2] << 8) |
			buff[3];
	dev->sector_size = (buff[4] << 24) | (buff[5] << 16) | (buff[6] << 8) |
			   buff[7];
